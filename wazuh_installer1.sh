...
apt-get update >> $log 
apt-get install -y elasticsearch=7.17.13 >> $log
...
apt-get update >> $log
apt-get install -y wazuh-manager=4.5.4-1 >> $log
systemctl daemon-reload >> $log
...
apt-get install -y filebeat=7.17.13 >> $log
...
apt-get install kibana=7.17.13 >> $log
...
cd /usr/share/kibana
sudo -u kibana /usr/share/kibana/bin/kibana-plugin install https://packages.wazuh.com/4.x/ui/kibana/wazuh_kibana-4.5.4_7.17.13-1.zip >> $log
